...
async def mtasks(prev_A):
    ...
    while cur_len < len(A):
        ...
        for start in range(0, len(A), 2 * cur_len):
            ...
            if flag:
                tasks.append(asyncio.create_task(merge(A, B, start, middle, finish, events[i], events[i + 1], new_events[-1])))
            else:
                tasks.append(asyncio.create_task(merge(B, A, start, middle, finish, events[i], events[i + 1], new_events[-1])))
            i += 2
            if finish == len(A):
                break
